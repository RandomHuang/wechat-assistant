...
function findContact(results, contactName) {
    const SKIP_TEXTS = ['联系人', '群聊', '聊天记录', '功能', '搜索网络结果', '查看全部', '公众号', '收藏'];

    // Find the y position of "搜索网络结果" divider
    // Results ABOVE it are actual contacts; results BELOW are web suggestions
    let dividerY = Infinity;
    for (const r of results) {
        if (r.text.includes('搜索网络结果')) {
            dividerY = r.y;
            break;
        }
    }

    // Only consider results above the divider
    const contactResults = results.filter(r => {
        if (r.y >= dividerY) return false;
        if (SKIP_TEXTS.includes(r.text)) return false;
        if (r.text.includes('包含') || r.text.includes('来自')) return false;
        return true;
    });
